...
public class CbnScService extends ServiceLifecycle {
    ...
    private final String mOfsVersion = CONFIG.getOfsVersionSc();
        ...
        try {
            ...
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                ...
                if (!Files.isDirectory(mInboundDir)) {
                    yLOGGER.log(Level.WARNING, LOG_PREFIX + "getNewTransactionIds: Inbound directory not found: {0}",
                            mInboundDir);
                    return pIds;
                }
                ...
                yLOGGER.log(Level.SEVERE, LOG_PREFIX + "getNewTransactionIds: Unknown adapter flag: {0}", mAdapterFlag);
            }
    ...
    private void processOfsRequest(String pRecordId, List<SynchronousTransactionData> pTransactionData,
        ...
        try {
            ...
            Map<String, String> pData = CbnScMapper.mapSecurityMasterToSc(pItem);
            ...
            yLOGGER.log(Level.INFO, LOG_PREFIX + "processOfsRequest: Transaction metadata cached for responseId={0}",
                    pResponseId);
            ...
            yLOGGER.log(Level.SEVERE, re, () -> LOG_PREFIX + "processOfsRequest: Runtime error for id=" + pRecordId);
            ...
            if (pOfsRequestDetailRecord != null && pOfsRequestDetailRecord.getMsgOut() != null) {
                ...
                if (pMsgOut != null && !pMsgOut.isEmpty()) {
                    ...
                    if (pMsgOut.contains("/-1/")) {
                        ...
                        pTransactRef = "ERROR";
                        yLOGGER.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: OFS Error detected: {0}", pMsgOut);
                    ...
                    if (MSG_SUCCESS.equalsIgnoreCase(pStatus) && "WMQ".equalsIgnoreCase(pMetadata.adapterMode)) {
                        ...
                        yLOGGER.log(Level.INFO, LOG_PREFIX + "checkOfsResponse: MQ message acknowledged for id={0}",
                    ...
                    yLOGGER.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: OFS message output is null or empty");
                }
    ...
    private JsonNode retrieveTransactionItem(String pId) throws java.io.IOException {
        ...
        if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "retrieveTransactionItem: Unknown adapter flag: {0}", mAdapterFlag);
        }
    ...
    private JsonNode processFileRecord(String pId) throws java.io.IOException {
        ...
        if (pRef == null) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: unexpected id format: {0}", pId);
            return null;
        }
    ...
    private JsonNode processWmqRecord(String pId) throws java.io.IOException {
        ...
        if (pRoot == null || !CbnScMapper.hasSecurityMaster(pRoot)) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: no SecurityMaster in message {0}",
                    pRef.messageId());
            return null;
        }
    ...
    private boolean buildScRecord(String pResponseId, Map<String, String> pData,
        ...
        try {
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "buildDepoRecord: Building Deposit SecurityMaster record for responseId: {0}",
                    pResponseId);
            ...
            if (sMnemonic.isEmpty() || sShortName.isEmpty() || sSecurityCurrency.isEmpty()) {
                yLOGGER.log(Level.SEVERE, "[CbnScService] Missing required fields");
                return false;
            }
            // Log extracted field values
            yLOGGER.log(Level.INFO,
                    "[CbnScService] Extracted fields - CNME={0}, SDES={1}, MMNE={2}, CDOM={3}, SDOM={4}",
                    new Object[] { sCompanyName, sDescription, sMnemonic, sCompanyDomicile, sSecurityDomicile });
            ...
            yLOGGER.log(Level.INFO, "[CbnScService] Adding record: {0}", pSecurityMasterRecord);
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "buildScRecord: Error building CbnPlacementDepositRecord", e);
            return false;
        } finally {
            // Cleanup
    ...
    private void publishResponse(String pId, String pStatus, String pMessage, String pTransactRef,
        ...
        try {
            ...
            yLOGGER.log(Level.SEVERE, e, () -> LOG_PREFIX + "publishResponse: Error publishing response for id=" + pId);
        }
    }
        ...
        try {
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                ...
                if (pRef != null) {
                    ...
                    yLOGGER.log(Level.WARNING, LOG_PREFIX + "persistToExcepts: could not parse FILE ref from id={0}",
                            pId);
                }
            } else if ("WMQ".equalsIgnoreCase(mAdapterFlag)) {
                MqItemRef pRef = CbnScAdapter.parseMqItemRef(pId);
                if (pRef != null) {
                    ...
                    yLOGGER.log(Level.WARNING, LOG_PREFIX + "persistToExcepts: could not parse WMQ ref from id={0}",
                            pId);
                }
            }
        } catch (Exception ex) {
            yLOGGER.log(Level.SEVERE, ex, () -> LOG_PREFIX + "persistToExcepts: persistToExcepts failed for id=" + pId);
        }
    }
    ...
    public static String extractPrefix(String pInput) {
        try {
            ...
            yLOGGER.log(Level.WARNING, e, () -> LOG_PREFIX + "extractPrefix: Error extracting prefix from: " + pInput);
            return MSG_UNKNOWN;
        }
    }
}