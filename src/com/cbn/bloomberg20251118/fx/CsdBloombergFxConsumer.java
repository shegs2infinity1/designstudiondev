...
/**
 * Title: CsdBloombergFxConsumer.java Author: CSD Development Team Date Created:
 * 2025-10-11
 * 
...
public final class CsdBloombergFxConsumer {
    ...
    // ==== STATIC CONSTANTS ====
    private static final Logger LOGGER = Logger.getLogger(CsdBloombergFxConsumer.class.getName());
    ...
    public static MQQueue createQueue(String p_queueName) throws JMSException {
        ...
        LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Created MQ queue: {0}", p_queueName);
        return queue;
    }
    ...
    public static void sendTextMessage(Connection p_connection, String p_queueName, String p_messageText)
        ...
        try {
            ...
            try {
                ...
                LOGGER.log(Level.WARNING, "[CsdBloombergFxConsumer] Failed to set encoding properties", e);
            }

            producer.send(message);
            LOGGER.log(Level.INFO, "[CsdBloombergFxConsumer] Sent message to queue: {0}", p_queueName);
    ...
    public static int consumeMessages(Connection p_connection, String p_queueName, MessageProcessor p_processor,
        ...
        LOGGER.log(Level.INFO, "[CsdBloombergFxConsumer] Starting consumption from queue: {0}", p_queueName);
        ...
        try {
            ...
            while (continueConsuming) {
                ...
                if (p_maxMessages > 0 && consumedCount >= p_maxMessages) {
                    LOGGER.log(Level.INFO, "[CsdBloombergFxConsumer] Max messages reached: {0}", p_maxMessages);
                    break;
                }
                ...
                if (message == null) {
                    LOGGER.log(Level.INFO, "[CsdBloombergFxConsumer] No more messages available (timeout)");
                    break;
                }
                ...
                try {
                    ...
                    LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Processing message: {0}", messageId);
                    ...
                    consumedCount++;
                    LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Successfully processed message {0}/{1}",
                            new Object[] { consumedCount, p_maxMessages > 0 ? p_maxMessages : "unlimited" });

                } catch (Exception e) {
                    LOGGER.log(Level.SEVERE, "[CsdBloombergFxConsumer] Error processing message", e);
                    // Continue consuming despite processing error
                }
            }

            LOGGER.log(Level.INFO, "[CsdBloombergFxConsumer] Consumption complete. Total messages: {0}", consumedCount);
    ...
    private static String extractMessageBody(Message p_message) throws JMSException {
        if (p_message instanceof TextMessage) {
            ...
            LOGGER.log(Level.WARNING, "[CsdBloombergFxConsumer] Non-text message received: {0}",
    ...
    private static void closeQuietly(MessageProducer p_producer) {
        if (p_producer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Error closing producer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(MessageConsumer p_consumer) {
        if (p_consumer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Error closing consumer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(Session p_session) {
        if (p_session != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergFxConsumer] Error closing session (ignored)", e);
            }
        }
    }
}