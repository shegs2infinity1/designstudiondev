...
/**
 * Title: CsdBloombergScPayload.java Author: CSD Development Team Date Created: 2025-11-06
 *
 ...
 *
 * Usage: Used by CsdBloombergScService to construct response payloads before publishing.
 */
public class CsdBloombergScPayload {

    private static final Logger LOGGER = Logger.getLogger(CsdBloombergScPayload.class.getName());
    private static final DateTimeFormatter TIMESTAMP_FMT = DateTimeFormatter.ofPattern(
            "yyyy-MM-dd'T'HH:mm:ss");
    private final ObjectMapper objectMapper;

    public CsdBloombergScPayload(ObjectMapper objectMapper) {
    ...
    public String buildResponse(String status, String message, String transactionRef, JsonNode originalItem) {
        try {
            ...
            LOGGER.log(Level.FINE, "[CsdBloombergScPayload] Built response: {0}", jsonResponse);

            return jsonResponse;

        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "[CsdBloombergScPayload] Error building response", e);
