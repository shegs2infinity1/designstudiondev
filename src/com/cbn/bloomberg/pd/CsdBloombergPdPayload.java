...
/**
 * Title: CsdBloombergPdPayload.java Author: CSD Development Team Date Created: 2025-11-06
 *
 ...
 *
 * Usage: Used by CsdBloombergPdService to construct response payloads before publishing.
 */
public class CsdBloombergPdPayload {

    private static final Logger LOGGER = Logger.getLogger(CsdBloombergPdPayload.class.getName());
    private static final DateTimeFormatter TIMESTAMP_FMT = DateTimeFormatter.ofPattern(
            "yyyy-MM-dd'T'HH:mm:ss");
    private final ObjectMapper objectMapper;

    public CsdBloombergPdPayload(ObjectMapper objectMapper) {
    ...
    public String buildResponse(String status, String message, String transactionRef,
        ...
        try {
            ...
            LOGGER.log(Level.FINE, "[CsdBloombergPdPayload] Built response: {0}", jsonResponse);

            return jsonResponse;

        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "[CsdBloombergPdPayload] Error building response", e);
