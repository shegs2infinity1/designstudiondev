 ...
 */
public class BloombergWmqManager {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("   Response Queue: " + mqProperties.getProperty("mq.response.queue", "(not configured)"));
            System.out.println();
    ...
    private static void runInteractiveMenu() {
        ...
        while (running) {
            ...
            System.out.println("6. Inspect response queue");
            System.out.println("7. Peek at first response message");
            System.out.println("8. Clear response queue");
            ...
            try {
                ...
                case "5":
                    ...
                    break;
                case "6":
                    inspectResponseQueue();
                    break;
                case "7":
                    peekFirstResponseMessage();
                    break;
                case "8":
                    clearResponseQueue(scanner);
                    break;
                case "9":
                    running = false;
                    System.out.println("\n👋 Exiting... Goodbye!");
                    break;
                default:
                    System.out.println("⚠️  Invalid option. Please select 1-9.");
                }
            } catch (Exception e) {
        ...
        try {
            ...
            while (messages.hasMoreElements()) {
                ...
                if (msg instanceof TextMessage) {
                    ...
                    javax.jms.BytesMessage bytesMsg = (javax.jms.BytesMessage) msg;
 ...
 // ========== OPERATION 4: PEEK FIRST MESSAGE (UPDATED) ==========
    private static void peekFirstMessage() throws Exception {
        ...
        try {
            ...
            if (body != null) {
                ...
                System.out.println("\n⚠️  Unsupported message type: " + msg.getJMSType());
            }
 ...
 // ========== OPERATION 6: INSPECT RESPONSE QUEUE ==========
    private static void inspectResponseQueue() throws Exception {
        String responseQueueName = mqProperties.getProperty("mq.response.queue");
        if (responseQueueName == null || responseQueueName.trim().isEmpty()) {
            System.out.println("❌ Response queue not configured in properties file.");
            return;
        }

        System.out.println("\n🔍 Inspecting response queue: " + responseQueueName);
        ...
        try {
            context = createMqContext();
            Queue queue = context.createQueue("queue:///" + responseQueueName);
            ...
            System.out.println("\n----------------------------------------");
            System.out.println("📊 Total messages in response queue: " + count);
    ...
    // ========== OPERATION 7: PEEK FIRST RESPONSE MESSAGE ==========
    private static void peekFirstResponseMessage() throws Exception {
        String responseQueueName = mqProperties.getProperty("mq.response.queue");
        if (responseQueueName == null || responseQueueName.trim().isEmpty()) {
            System.out.println("❌ Response queue not configured in properties file.");
            return;
        ...
        System.out.println("\n👀 Peeking at first response message from: " + responseQueueName);
        ...
        try {
            ...
            if (!messages.hasMoreElements()) {
                System.out.println("⚠️  Response queue is empty.");
                return;
            }

            Message msg = (Message) messages.nextElement();

            System.out.println("\n📄 First Response Message Details:");
            System.out.println("----------------------------------------");
            System.out.println("Message ID: " + msg.getJMSMessageID());
            System.out.println("Timestamp: " + new java.util.Date(msg.getJMSTimestamp()));
            System.out.println("Type: " + msg.getClass().getSimpleName());
            ...
            if (body != null) {
                ...
                System.out.println("\n⚠️  Unsupported message type: " + msg.getClass().getSimpleName());
            }
    ...
    // ========== OPERATION 8: CLEAR RESPONSE QUEUE ==========
    private static void clearResponseQueue(Scanner scanner) throws Exception {
        String responseQueueName = mqProperties.getProperty("mq.response.queue");
        if (responseQueueName == null || responseQueueName.trim().isEmpty()) {
            System.out.println("❌ Response queue not configured in properties file.");
            return;
        ...
        System.out.println("\n⚠️  WARNING: This will delete ALL messages from the response queue!");
        System.out.println("   Queue: " + responseQueueName);
        System.out.print("Type 'CONFIRM' to proceed: ");
        String confirm = scanner.nextLine().trim();
        ...
        try {
            ...
            System.out.println("\n✅ Response queue cleared successfully!");
            System.out.println("   Deleted messages: " + deletedCount);
    ...
    private static void loadMqConfiguration() throws IOException, JMSException {
        ...
        InputStream in = BloombergWmqManager.class.getClassLoader().getResourceAsStream(PROPERTIES_FILE);
        if (in != null) {
