...
public class WmqBmrgPcProducer {
    ...
    public static void main(String[] args) {
        ...
        String mqHost = props.getProperty("pd.mq.host", "172.22.105.46");
        int mqPort = Integer.parseInt(props.getProperty("pd.mq.port", "1414"));
        String mqChannel = props.getProperty("pd.mq.channel", "DEV.APP.SVRCONN");
        String mqQmgr = props.getProperty("pd.mq.qmgr", "QM_BLOOMBERG");
        String mqQueue = props.getProperty("pd.mq.inbound.queue", "PD.INBOUND.QUEUE");
        String mqUser = props.getProperty("pd.mq.user", "");
        String mqPass = props.getProperty("pd.mq.password", "");

        // Expand placeholders for outbound directory
        String rawDir = props.getProperty("pd.fs.outbound.dir", "${pd.fs.bloomberg.base.dir}\\OUT\\PD");
        ...
        if (!Files.isDirectory(outwardDir)) {
            ...
            System.err.println(
                    "   You can configure this in " + PROPERTIES_FILE + " using property: pd.fs.outbound.dir");
            return;
        }
    ...
    private static Properties loadProperties() {
        ...
        try {
            ...
            if (is == null) is = WmqBmrgPcProducer.class.getClassLoader().getResourceAsStream(PROPERTIES_FILE);
            if (is == null) is = WmqBmrgPcProducer.class.getResourceAsStream("/" + PROPERTIES_FILE);
            if (is == null) {
