 ...
 * 
 * Configuration is loaded from bloombergro.properties in the resources directory.
 */
public class WmqBmrgScProducer {

    private static final String PROPERTIES_FILE = "sc-bloomberg.properties";
    private static final String FILE_GLOB = "*.json";

    public static void main(String[] args) {
        System.out.println("=== Bloomberg MQ Producer (SC Module) ===");
        System.out.println();
        ...
        if (props == null) {
            ...
            System.err.println(
                    "   Make sure the file is in your resources directory (src/resources or src/test/resources)");
            return;
        }

        // Extract MQ configuration
        String mqHost = props.getProperty("sc.mq.host", "172.22.105.46");
        int mqPort = Integer.parseInt(props.getProperty("sc.mq.port", "1414"));
        String mqChannel = props.getProperty("sc.mq.channel", "DEV.APP.SVRCONN");
        String mqQmgr = props.getProperty("sc.mq.qmgr", "QM_BLOOMBERG");
        String mqQueue = props.getProperty("sc.mq.inbound.queue", "SC.INBOUND.QUEUE");
        String mqUser = props.getProperty("sc.mq.user", "");
        String mqPass = props.getProperty("sc.mq.password", "");

        // Expand placeholders for outbound directory
        String rawDir = props.getProperty("sc.fs.outbound.dir", "${sc.fs.bloomberg.base.dir}\\OUT\\SC");
        ...
        System.out.println("   Auth Mode: " + (mqUser.isEmpty() ? "No Auth (MCAUSER)" : "User/Password"));
        ...
        if (!Files.isDirectory(outwardDir)) {
            ...
            System.err.println("   You can configure this in " + PROPERTIES_FILE
                    + " using property: sc.fs.outbound.dir");
            return;
        }
            ...
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(outwardDir, FILE_GLOB)) {
                for (Path file : stream) {
                    try {
                        ...
                        errorCount++;
                        System.err.println(
                                "❌ Error reading file " + file.getFileName() + ": " + e.getMessage());
                    } catch (JMSException e) {
                        errorCount++;
                        System.err.println(
                                "❌ Error sending file " + file.getFileName() + ": " + e.getMessage());
                    }
                }
            }
    ...
    /**
     * Expands ${property} placeholders recursively using loaded properties, environment variables,
    ...
    private static Properties loadProperties() {
        ...
        try {
            ...
            if (is == null)
                is = WmqBmrgScProducer.class.getClassLoader().getResourceAsStream(PROPERTIES_FILE);
            if (is == null) is = WmqBmrgScProducer.class.getResourceAsStream("/" + PROPERTIES_FILE);
            if (is == null) {
                System.err.println("⚠️  Properties file not found in classpath: " + PROPERTIES_FILE);
                return null;
            }
            props.load(is);
