...
import com.cbn.bloomberg.ft.CsdBloombergFtAdapter.FileItemRef;
import com.cbn.bloomberg.ft.CsdBloombergFtAdapter.MqItemRef;
...
public class CsdBloombergFtService extends ServiceLifecycle {
    ...
    private static final CsdBloombergFtProperties CONFIG = CsdBloombergFtProperties.getInstance();
    ...
    // Log prefix
    private static final String LOG_PREFIX = "[CsdBloombergFtService] ";
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    // ==== CONFIGURATION ====
    private final String mAdapterFlag = CONFIG.getFtAdapterMode();
    private final String mOfsFunction = CONFIG.getFtOfsFunction();
    private final String mOfsSource = CONFIG.getFtOfsSource();
    private final String pOfsVersion = CONFIG.getFtOfsVersion();
    private final String mFilePattern = CONFIG.getFtFilePattern();
    private final Path mInboundDir = Paths.get(CONFIG.geFtInboundDir());
    private final Path mProcessedDir = Paths.get(CONFIG.getFtDoneDir());
    private final Path mExceptsDir = Paths.get(CONFIG.getFtErrorDir());

    // ==== INSTANCE VARIABLES ====
    private String mCompanyId = "BNK";
    private Session mSession = null;
    private DataAccess mDataAccess = null;
    private final CsdBloombergFtPayload mPayloadHandler = new CsdBloombergFtPayload(OBJECT_MAPPER);
    private final CsdBloombergFtProducer mProducer = new CsdBloombergFtProducer();
    ...
    private List<String> getNewTransactionIds() {
        ...
        try {
            ...
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                ...
                pIds.addAll(CsdBloombergFtAdapter.extractIdsFromDirectoryAndMove(mInboundDir, mFilePattern,
                ...
                pIds.addAll(CsdBloombergFtAdapter.extractIdsFromMq(OBJECT_MAPPER));
    ...
    private void processOfsRequest(String pRecordId, List<SynchronousTransactionData> pTransactionData,
        ...
        try {
            ...
            Map<String, String> pData = CsdBloombergFtMapper.mapFundsMovementToFt(pItem);
            ...
            if (pOfsRequestDetailRecord != null && pOfsRequestDetailRecord.getMsgOut() != null) {
                ...
                if (pMsgOut != null && !pMsgOut.isEmpty()) {
                    ...
                    if (MSG_SUCCESS.equalsIgnoreCase(pStatus) && "WMQ".equalsIgnoreCase(pMetadata.adapterMode)) {
                        CsdBloombergFtAdapter.acknowledgeMqMessage(pMetadata.originalId);
    ...
    private JsonNode processFileRecord(String pId) throws java.io.IOException {
        FileItemRef pRef = CsdBloombergFtAdapter.parseFileItemRef(pId);
        if (pRef == null) {
            LOG.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: unexpected id format: {0}", pId);
            return null;
        }

        JsonNode pRoot = CsdBloombergFtAdapter.readRoot(pRef.file(), OBJECT_MAPPER);
        if (!CsdBloombergFtMapper.hasFundsMovement(pRoot)) {
            LOG.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: no FUNDS_MOVEMENT in {0}",
                    pRef.file().getFileName());
            return null;
        }

        return CsdBloombergFtMapper.getFundsMovementAt(pRoot, pRef.index());
    }
    ...
    private JsonNode processWmqRecord(String pId) throws java.io.IOException {
        MqItemRef pRef = CsdBloombergFtAdapter.parseMqItemRef(pId);
        if (pRef == null) {
            LOG.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: unexpected id format: {0}", pId);
            return null;
        }

        JsonNode pRoot = CsdBloombergFtAdapter.readMqMessage(pRef.messageId(), OBJECT_MAPPER);
        if (pRoot == null || !CsdBloombergFtMapper.hasFundsMovement(pRoot)) {
    ...
    private boolean buildFtRecord(String pResponseId, Map<String, String> pData,
        ...
        try {
            ...
            LOG.log(Level.INFO, "[CsdBloombergFtService] Adding record: {0}", pFtRecord);
    ...
    private void persistToExcepts(String pId, String pReason) {
        try {
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                FileItemRef pRef = CsdBloombergFtAdapter.parseFileItemRef(pId);
                if (pRef != null) {
                    CsdBloombergFtAdapter.persistFailedFileItem(pRef, mExceptsDir, pReason);
                } else {
                ...
                MqItemRef pRef = CsdBloombergFtAdapter.parseMqItemRef(pId);
                if (pRef != null) {
                    CsdBloombergFtAdapter.persistFailedMqItem(pRef, mExceptsDir, pReason);
                } else {
