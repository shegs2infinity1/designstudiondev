...
import com.cbn.bloomberg.ro.CsdBloombergRoAdapter.FileItemRef;
import com.cbn.bloomberg.ro.CsdBloombergRoAdapter.MqItemRef;
...
/**
 * Title: CsdBloombergRoService.java Author: CSD Development Team Date Created: 2025-10-11 Last
 * Modified: 2025-11-09
 *
 * Purpose: Bloomberg RO Service Hook supporting dual ingestion modes: FILE or WMQ (IBM MQ). The
 * adapter mode is controlled by bloomberg.properties.
 *
 * Two-Phase Processing Pattern: - Phase 1 (PROCESS.RO): Submit OFS request to T24 with responseId -
...
public class CsdBloombergRoService extends ServiceLifecycle {

  // ==== CONSTANTS ====
  private static final Logger LOG = CsdBloombergTafjLogger.forClass(CsdBloombergRoService.class);
  private static final CsdBloombergRoProperties CONFIG = CsdBloombergRoProperties.getInstance();

  // Control list constants
  private static final String CONTROL_PROCESS_FT = "PROCESS.RO";
  ...
  // Log prefix
  private static final String LOG_PREFIX = "[CsdBloombergRoService] ";
  private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

  // ==== CONFIGURATION ====
  private final String mAdapterFlag = CONFIG.getRoAdapterMode();
  private final String mOfsFunction = CONFIG.getRoOfsFunction();
  private final String mOfsSource = CONFIG.getRoOfsSource();
  private final String pOfsVersion = CONFIG.getRoOfsVersion();
  private final String mFilePattern = CONFIG.getRoFilePattern();
  private final Path mInboundDir = Paths.get(CONFIG.getRoInboundDir());
  private final Path mProcessDir = Paths.get(CONFIG.getRoDoneDir());
  private final Path mExceptsDir = Paths.get(CONFIG.getRoErrorDir());

  // ==== INSTANCE VARIABLES ====
  private String mCompanyId = "BNK";
  private Session mSession = null;
  private DataAccess mDataAccess = null;
  private final CsdBloombergRoPayload mPayloadHandler = new CsdBloombergRoPayload(OBJECT_MAPPER);
  private final CsdBloombergRoProducer mProducer = new CsdBloombergRoProducer();
   ...
   * processing: - Phase 1: PROCESS.RO - Get new transactions from FILE/WMQ - Phase 2:
  ...
  private List<String> getNewTransactionIds() {
    ...
    try {
      ...
      if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
        ...
        pIds.addAll(CsdBloombergRoAdapter.extractIdsFromDirectoryAndMove(mInboundDir, mFilePattern,
        ...
        pIds.addAll(CsdBloombergRoAdapter.extractIdsFromMq(OBJECT_MAPPER));

      } else {
        LOG.log(Level.SEVERE, LOG_PREFIX + "getNewTransactionIds: Unknown adapter flag: {0}",
            mAdapterFlag);
      }

      LOG.log(Level.INFO, LOG_PREFIX + "=== getNewTransactionIds() COMPLETE - Found {0} IDs ===",
          pIds.size());
  ...
  public void updateRecord(String id, ServiceData serviceData, String controlItem,
    ...
    try {
      ...
      if (controlItem == null || controlItem.isEmpty()) {
        LOG.log(Level.WARNING,
            LOG_PREFIX + "updateRecord: Control item is null or empty, defaulting to PROCESS.RO");
      ...
      switch (controlItem) {
        ...
        default:
          LOG.log(Level.WARNING, LOG_PREFIX + "updateRecord: Unrecognized control item: {0}",
              controlItem);
          break;
      }
  ...
  private void processOfsRequest(String pRecordId,
    ...
    try {
      ...
      Map<String, String> pData = CsdBloombergRoMapper.mapRepositoryToRo(pItem);
      ...
      LOG.log(Level.INFO,
          LOG_PREFIX + "processOfsRequest: Successfully prepared OFS transaction for RO update");
  ...
  private void checkOfsResponse(String pResponseId) {
    ...
    if (pMetadata == null) {
      LOG.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: No metadata found for responseId={0}",
          pResponseId);
      return;
    }

    try {
      LOG.log(Level.INFO, LOG_PREFIX + "checkOfsResponse: Checking OFS response for record ID: {0}",
          pResponseId);
      ...
      if (pOfsRequestDetailRecord != null && pOfsRequestDetailRecord.getMsgOut() != null) {
        ...
        if (pMsgOut != null && !pMsgOut.isEmpty()) {
          ...
          if (pMsgOut.contains("/-1/")) {
            ...
            pTransactRef = "ERROR";
            LOG.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: OFS Error detected: {0}",
                pMsgOut);
          ...
          if (MSG_SUCCESS.equalsIgnoreCase(pStatus)
            ...
            CsdBloombergRoAdapter.acknowledgeMqMessage(pMetadata.originalId);
      ...
      LOG.log(Level.SEVERE, LOG_PREFIX + "checkOfsResponse: Unexpected error in checkOfsResponse",
          e);
  ...
  private JsonNode processFileRecord(String pId) throws java.io.IOException {
    FileItemRef pRef = CsdBloombergRoAdapter.parseFileItemRef(pId);
    if (pRef == null) {
      LOG.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: unexpected id format: {0}",
          pId);
      return null;
    }

    JsonNode pRoot = CsdBloombergRoAdapter.readRoot(pRef.file(), OBJECT_MAPPER);
    if (!CsdBloombergRoMapper.hasRepository(pRoot)) {
      LOG.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: no REPO in {0}",
          pRef.file().getFileName());
      return null;
    }

    return CsdBloombergRoMapper.getRepositoryAt(pRoot, pRef.index());
  }
  ...
  private JsonNode processWmqRecord(String pId) throws java.io.IOException {
    MqItemRef pRef = CsdBloombergRoAdapter.parseMqItemRef(pId);
    if (pRef == null) {
      LOG.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: unexpected id format: {0}",
          pId);
      return null;
    }

    JsonNode pRoot = CsdBloombergRoAdapter.readMqMessage(pRef.messageId(), OBJECT_MAPPER);
    if (pRoot == null || !CsdBloombergRoMapper.hasRepository(pRoot)) {
      LOG.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: no REPO in message {0}",
          pRef.messageId());
      return null;
    }
  ...
  private boolean buildFtRecord(String pResponseId, Map<String, String> pData,
    ...
    try {
      ...
      LOG.log(Level.INFO,
          LOG_PREFIX + "buildFtRecord: Building fundstransfer record for responseId: {0}",
          pResponseId);
      ...
      if (drawDownAcct.isEmpty() || repoCurrency.isEmpty() || valueDate.isEmpty()) {
        LOG.log(Level.SEVERE, "[CsdBloombergRoService] Missing required fields");
        return false;
      }
      // Log extracted field values
      LOG.log(Level.INFO,
          "[CsdBloombergRoService] Extracted fields - CPTY={0}, DCCY={1}, TDTE={2}, VDTE={3}, MDTE={4}, AMT1={5}, AMT2={6}, RRTE={7}, RPRD={8}, DDAC={9}, DIAC={10}, TINT={11}",
          new Object[] { counterParty, repoCurrency, tradeDate, valueDate, maturityDate,
              principalAmt1, principalAmt2, repoRate, repoProduct, drawDownAcct, drawInAcct,
              interestRate });
      ...
      LOG.log(Level.INFO, "[CsdBloombergRoService] Adding record: {0}", pRepoRecord);
    ...
    try {
      if (pOfsResponse.contains("/") && pOfsResponse.contains(",")) {
        ...
        if (firstSlash > 0 && secondSlash > firstSlash && commaAfter > secondSlash) {
          ...
          LOG.log(Level.INFO, LOG_PREFIX + "parseTransactionRef: Parsed transaction reference: {0}",
              pTxnRef);
          return pTxnRef;
        }
      }
  ...
  private void persistToExcepts(String pId, String pReason) {
    try {
      if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
        FileItemRef pRef = CsdBloombergRoAdapter.parseFileItemRef(pId);
        if (pRef != null) {
          CsdBloombergRoAdapter.persistFailedFileItem(pRef, mExceptsDir, pReason);
        } else {
          LOG.log(Level.WARNING,
        ...
        MqItemRef pRef = CsdBloombergRoAdapter.parseMqItemRef(pId);
        if (pRef != null) {
          CsdBloombergRoAdapter.persistFailedMqItem(pRef, mExceptsDir, pReason);
        } else {
          LOG.log(Level.WARNING,
