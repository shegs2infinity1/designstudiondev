...
public final class CbnScConsumer {

    // ==== STATIC CONSTANTS ====
    private static final Logger LOGGER = Logger.getLogger(CbnScConsumer.class.getName());

    /**
     * Private constructor to prevent instantiation of utility class.
     */
    private CbnScConsumer() {
    ...
    public static MQQueue createQueue(String pQueueName) throws JMSException {
        ...
        LOGGER.log(Level.FINE, "[CbnScConsumer] Created MQ queue: {0}", pQueueName);
        return queue;
    }
    ...
    public static void sendTextMessage(Connection pConnection, String pQueueName,
        ...
        try {
            ...
            LOGGER.log(Level.INFO, "[CbnScConsumer] Sent message to queue: {0}", pQueueName);
    ...
    public static int consumeMessages(Connection pConnection, String pQueueName,
        ...
        LOGGER.log(Level.INFO, "[CbnScConsumer] Starting consumption from queue: {0}", pQueueName);
        ...
        try {
            ...
            while (continueConsuming) {
                ...
                if (pMaxMessages > 0 && consumedCount >= pMaxMessages) {
                    LOGGER.log(Level.INFO, "[CbnScConsumer] Max messages reached: {0}",
                            pMaxMessages);
                    break;
                }
                ...
                if (message == null) {
                    LOGGER.log(Level.INFO, "[CbnScConsumer] No more messages available (timeout)");
                    break;
                }
                ...
                try {
                    ...
                    LOGGER.log(Level.FINE, "[CbnScConsumer] Processing message: {0}", messageId);
            ...
            LOGGER.log(Level.INFO, "[CbnScConsumer] Consumption complete. Total messages: {0}",
                    consumedCount);
    ...
    private static String extractMessageBody(Message pMessage) throws JMSException {
        if (pMessage instanceof TextMessage) {
            ...
            LOGGER.log(Level.WARNING, "[CbnScConsumer] Non-text message received: {0}",
    ...
    private static void closeQuietly(MessageProducer pProducer) {
        if (pProducer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnScConsumer] Error closing producer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(MessageConsumer pConsumer) {
        if (pConsumer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnScConsumer] Error closing consumer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(Session pSession) {
        if (pSession != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnScConsumer] Error closing session (ignored)", e);
            }
        }
    }
}