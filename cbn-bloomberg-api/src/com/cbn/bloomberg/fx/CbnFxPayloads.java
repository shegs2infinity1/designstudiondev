 ...
 *
 * @usage:
 *         Used by CbnFxService to construct response payloads before
 *         publishing.
 */
public class CbnFxPayloads {

    private static final Logger yLogger = Logger.getLogger(CbnFxPayloads.class.getName());
    private static final DateTimeFormatter TIMESTAMP_FMT = DateTimeFormatter.ofPattern(
            "yyyy-MM-dd'T'HH:mm:ss");

    private final ObjectMapper objectMapper;

    public CbnFxPayloads(ObjectMapper objectMapper) {
    ...
    public String buildResponse(String status, String message, String transactionRef,
        ...
        try {
            ...
            yLogger.log(Level.FINE, "[CbnFxPayloads] Built response: {0}", jsonResponse);
            return jsonResponse;

        } catch (Exception e) {
            yLogger.log(Level.SEVERE, "[CbnFxPayloads] Error building response", e);
