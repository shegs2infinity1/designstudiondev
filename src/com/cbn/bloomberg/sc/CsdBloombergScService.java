...
import com.cbn.bloomberg.sc.CsdBloombergScAdapter.FileItemRef;
import com.cbn.bloomberg.sc.CsdBloombergScAdapter.MqItemRef;
 ...
 * =============================================================================
 * 
 * CSD API Title: CsdBloombergScService.java Author: CSD Development Team Created: 2026-01-07 Last
 * Modified: 2026-01-07
 * =============================================================================
 *
 * PURPOSE: Bloomberg SC Service Hook supporting dual ingestion modes: FILE or WMQ (IBM MQ). The
 * adapter mode is controlled by bloomberg.properties.
 *
 * TWO-PHASE PROCESSING PATTERN: Phase 1 (PROCESS.SC): Submit OFS request to T24 with responseId
 ...
 * 
 * 2025-11-09 | [initials] | Updated to handle single SecurityMaster object format 2025-10-11 |
 * [initials] | Initial creation
 * =============================================================================
 * 
 */

public class CsdBloombergScService extends ServiceLifecycle {

    // ==== CONSTANTS ====
    private static final Logger yLOGGER = CsdBloombergTafjLogger.forClass(CsdBloombergScService.class);
    private static final CsdBloombergScProperties CONFIG = CsdBloombergScProperties.getInstance();
    ...
    // Log prefix
    private static final String LOG_PREFIX = "[CsdBloombergScService] ";
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    // ==== CONFIGURATION ====
    private final String mAdapterFlag = CONFIG.getAdapterMode();
    private final String mOfsSource = CONFIG.getOfsSource();
    private final String mOfsVersion = CONFIG.getOfsVersion();
    private final String mOfsFunction = CONFIG.getOfsFunction();

    private final Path mInboundDir = Paths.get(CONFIG.getFsInboundDir());
    private final Path mProcessDir = Paths.get(CONFIG.getFsDoneDir());
    private final Path mExceptsDir = Paths.get(CONFIG.getFsErrorDir());
    private final String mFilePattern = CONFIG.getFsFilePattern();

    // ==== INSTANCE VARIABLES ====
    private String mCompanyId = "BNK";
    private Session mSession = null;
    private DataAccess mDataAccess = null;
    private final CsdBloombergScPayload mPayloadHandler = new CsdBloombergScPayload(OBJECT_MAPPER);
    private final CsdBloombergScProducer mProducer = new CsdBloombergScProducer();
     ...
     * processing: - Phase 1: PROCESS.SC - Get new transactions from FILE/WMQ - Phase 2:
    ...
    /**
     * Gets new transaction IDs from FILE or WMQ sources. NOW HANDLES SINGLE SecurityMaster OBJECT
     * FORMAT
     */
    private List<String> getNewTransactionIds() {
        ...
        try {
            ...
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                yLOGGER.log(Level.INFO, LOG_PREFIX + "getNewTransactionIds: FILE mode: scanning {0}",
                        mInboundDir);
                ...
                pIds.addAll(CsdBloombergScAdapter.extractIdsFromDirectoryAndMove(mInboundDir, mFilePattern,
                ...
                pIds.addAll(CsdBloombergScAdapter.extractIdsFromMq(OBJECT_MAPPER));

            } else {
                yLOGGER.log(Level.SEVERE, LOG_PREFIX + "getNewTransactionIds: Unknown adapter flag: {0}",
                        mAdapterFlag);
            }

            yLOGGER.log(Level.INFO, LOG_PREFIX + "=== getNewTransactionIds() COMPLETE - Found {0} IDs ===",
                    pIds.size());
    ...
    private List<String> getPendingResponseIds() {
        ...
        try {
            ...
            yLOGGER.log(Level.INFO, LOG_PREFIX + "getPendingResponseIds: Found {0} pending responses",
                    pResponseIds.size());
    ...
    public void updateRecord(String id, ServiceData serviceData, String controlItem,
        ...
        try {
            ...
            if (controlItem == null || controlItem.isEmpty()) {
                yLOGGER.log(Level.WARNING,
                        LOG_PREFIX + "updateRecord: Control item is null or empty, defaulting to PROCESS.SC");
            ...
            switch (controlItem) {
                ...
                default:
                    yLOGGER.log(Level.WARNING, LOG_PREFIX + "updateRecord: Unrecognized control item: {0}",
                            controlItem);
                    break;
            }
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "updateRecord: Unexpected error in updateRecord", e);
        }
    }

    /**
     * Phase 1: Process OFS request - retrieve transaction, map to SC record, submit to T24. NOW
     * HANDLES SINGLE SecurityMaster OBJECT FORMAT
     */
    private void processOfsRequest(String pRecordId, List<SynchronousTransactionData> pTransactionData,
        ...
        try {
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "processOfsRequest: Processing OFS request for record ID: {0}", pRecordId);

            // Step 1: Retrieve the item based on adapter mode
            JsonNode pItem = retrieveTransactionItem(pRecordId);
            if (pItem == null) {
                pMessage = "Item not found or invalid SecurityMaster";
            ...
            pOriginalItem = pItem;
            yLOGGER.log(Level.FINE, LOG_PREFIX + "processOfsRequest: Retrieved item for id={0}", pRecordId);

            // Step 2: Map JSON to SC field map
            Map<String, String> pData = CsdBloombergScMapper.mapSecurityMasterToSc(pItem);
            ...
            yLOGGER.log(Level.FINE, LOG_PREFIX + "processOfsRequest: Mapped data for id={0}", pRecordId);

            // Step 3: Build responseId
            pResponseId = buildResponseId(pRecordId);
            yLOGGER.log(Level.INFO, LOG_PREFIX + "processOfsRequest: Built responseId: {0}", pResponseId);

            // Step 4: Build and populate SC record
            boolean pSuccess = buildScRecord(pResponseId, pData, pTransactionData, pRecords);
            if (!pSuccess) {
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "processOfsRequest: Successfully prepared OFS transaction for SC update");
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "processOfsRequest: Transaction metadata cached for responseId={0}",
                    pResponseId);
    ...
    private void checkOfsResponse(String pResponseId) {
        ...
        if (pMetadata == null) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: No metadata found for responseId={0}",
                    pResponseId);
            return;
        }

        try {
            yLOGGER.log(Level.INFO, LOG_PREFIX + "checkOfsResponse: Checking OFS response for record ID: {0}",
                    pResponseId);
            ...
            yLOGGER.log(Level.INFO, LOG_PREFIX + "checkOfsResponse: OFS record exists: {0}", pExists);
            ...
            if (pOfsRequestDetailRecord != null && pOfsRequestDetailRecord.getMsgOut() != null) {
                ...
                if (pMsgOut != null && !pMsgOut.isEmpty()) {
                    yLOGGER.log(Level.INFO,
                            LOG_PREFIX + "checkOfsResponse: OFS response message retrieved successfully");
                    ...
                    if (pMsgOut.contains("/-1/")) {
                        ...
                        pTransactRef = "ERROR";
                        yLOGGER.log(Level.WARNING, LOG_PREFIX + "checkOfsResponse: OFS Error detected: {0}",
                                pMsgOut);
                        ...
                        pStatus = MSG_SUCCESS;
                        pMessage = "SecurityMaster transaction processed successfully";
                        yLOGGER.log(Level.INFO,
                    ...
                    if (MSG_SUCCESS.equalsIgnoreCase(pStatus)
                        ...
                        CsdBloombergScAdapter.acknowledgeMqMessage(pMetadata.originalId);
                        yLOGGER.log(Level.INFO,
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "checkOfsResponse: Error checking OFS response", e);
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "checkOfsResponse: Unexpected error in checkOfsResponse",
                    e);
     ...
     * SecurityMaster OBJECT FORMAT
     */
    private JsonNode retrieveTransactionItem(String pId) throws java.io.IOException {
        yLOGGER.log(Level.INFO, LOG_PREFIX + "=== retrieveTransactionItem() START for ID: {0} ===", pId);
        ...
        if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "retrieveTransactionItem: Unknown adapter flag: {0}",
                    mAdapterFlag);
        }
    ...
    private JsonNode processFileRecord(String pId) throws java.io.IOException {
        FileItemRef pRef = CsdBloombergScAdapter.parseFileItemRef(pId);
        if (pRef == null) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: unexpected id format: {0}",
                    pId);
            return null;
        }

        JsonNode pRoot = CsdBloombergScAdapter.readRoot(pRef.file(), OBJECT_MAPPER);
        if (!CsdBloombergScMapper.hasSecurityMaster(pRoot)) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: no SecurityMaster in {0}",
                    pRef.file().getFileName());
            return null;
        }

        return CsdBloombergScMapper.getSecurityMasterAt(pRoot, pRef.index());
    }
    ...
    private JsonNode processWmqRecord(String pId) throws java.io.IOException {
        MqItemRef pRef = CsdBloombergScAdapter.parseMqItemRef(pId);
        if (pRef == null) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: unexpected id format: {0}",
                    pId);
            return null;
        }

        JsonNode pRoot = CsdBloombergScAdapter.readMqMessage(pRef.messageId(), OBJECT_MAPPER);
        if (pRoot == null || !CsdBloombergScMapper.hasSecurityMaster(pRoot)) {
            yLOGGER.log(Level.WARNING,
    ...
    private boolean buildScRecord(String pResponseId, Map<String, String> pData,
        ...
        try {
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX
                            + "buildDepoRecord: Building Deposit SecurityMaster record for responseId: {0}",
                    pResponseId);
            ...
            String sIntPaymentDate = pData.getOrDefault("SCTE", "");
            ...
            if (sMnemonic.isEmpty() || sShortName.isEmpty() || sSecurityCurrency.isEmpty()) {
                yLOGGER.log(Level.SEVERE, "[CsdBloombergScService] Missing required fields");
                return false;
            }
            // Log extracted field values
            yLOGGER.log(Level.INFO,
                    "[CsdBloombergScService] Extracted fields - CNME={0}, SDES={1}, MMNE={2}, CDOM={3}, SDOM={4}",
                    new Object[] { sCompanyName, sDescription, sMnemonic, sCompanyDomicile,
                            sSecurityDomicile });
            ...
            yLOGGER.log(Level.INFO, "[CsdBloombergScService] Adding record: {0}", pSecurityMasterRecord);
            ...
            yLOGGER.log(Level.INFO, LOG_PREFIX
                    + "buildScRecord: SECURITY_MASTER record populated successfully Transaction data added with responseId={0}",
                    pResponseId);
            return true;

        } catch (Exception e) {
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "buildScRecord: Error building CbnPlacementDepositRecord",
                    e);
            return false;
        } finally {
            // Cleanup
        ...
        try {
            if (pOfsResponse.contains("/") && pOfsResponse.contains(",")) {
                ...
                if (firstSlash > 0 && secondSlash > firstSlash && commaAfter > secondSlash) {
                    ...
                    yLOGGER.log(Level.INFO,
                            LOG_PREFIX + "parseTransactionRef: Parsed transaction reference: {0}", pTxnRef);
                    return pTxnRef;
                }
            }
            ...
            yLOGGER.log(Level.WARNING,
                    LOG_PREFIX + "parseTransactionRef: Error parsing transaction reference from OFS response",
                    e);
            return MSG_UNKNOWN;
        }
    }
    ...
    /**
     * Builds response and publishes using CsdBloombergScPayload and CsdBloombergScProducer.
     */
    private void publishResponse(String pId, String pStatus, String pMessage, String pTransactRef,
        ...
        try {
            String pJsonResponse = mPayloadHandler.buildResponse(pStatus, pMessage, pTransactRef,
                    pOriginalItem);
            yLOGGER.log(Level.INFO, LOG_PREFIX + "publishResponse: Built response: {0}", pJsonResponse);
    ...
    private void persistToExcepts(String pId, String pReason) {
        try {
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                FileItemRef pRef = CsdBloombergScAdapter.parseFileItemRef(pId);
                if (pRef != null) {
                    CsdBloombergScAdapter.persistFailedFileItem(pRef, mExceptsDir, pReason);
                } else {
                    yLOGGER.log(Level.WARNING,
                            LOG_PREFIX + "persistToExcepts: could not parse FILE ref from id={0}", pId);
                }
            } else if ("WMQ".equalsIgnoreCase(mAdapterFlag)) {
                MqItemRef pRef = CsdBloombergScAdapter.parseMqItemRef(pId);
                if (pRef != null) {
                    CsdBloombergScAdapter.persistFailedMqItem(pRef, mExceptsDir, pReason);
                } else {
                    yLOGGER.log(Level.WARNING,
                            LOG_PREFIX + "persistToExcepts: could not parse WMQ ref from id={0}", pId);
                }
            }
        } catch (Exception ex) {
