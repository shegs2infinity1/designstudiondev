...
import com.cbn.bloomberg.pd.CsdBloombergPdAdapter.FileItemRef;
import com.cbn.bloomberg.pd.CsdBloombergPdAdapter.MqItemRef;
...
/**
 * =============================================================================
 * CSD API Title: CsdBloombergPdService.java
...
public class CsdBloombergPdService extends ServiceLifecycle {

    // ==== CONSTANTS ====
    private static final Logger yLOGGER = CsdBloombergTafjLogger.forClass(CsdBloombergPdService.class);
    private static final CsdBloombergPdProperties CONFIG = CsdBloombergPdProperties.getInstance();
    ...
    // Log prefix
    private static final String LOG_PREFIX = "[CsdBloombergPdService] ";
    ...
    private final CsdBloombergPdPayload mPayloadHandler = new CsdBloombergPdPayload(OBJECT_MAPPER);
    private final CsdBloombergPdProducer mProducer = new CsdBloombergPdProducer();
    ...
    */
    ...
    /**
    * Gets new transaction IDs from FILE or WMQ sources. NOW HANDLES SINGLE PLACEMENTS OBJECT FORMAT
    */
    private List<String> getNewTransactionIds() {
        ...
        try {
            ...
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                yLOGGER.log(Level.INFO, LOG_PREFIX + "getNewTransactionIds: FILE mode: scanning {0}", mInboundDir);

                if (!Files.isDirectory(mInboundDir)) {
                    yLOGGER.log(Level.WARNING,
                            LOG_PREFIX + "getNewTransactionIds: Inbound directory not found: {0}", mInboundDir);
                    return pIds;
                }

                pIds.addAll(CsdBloombergPdAdapter.extractIdsFromDirectoryAndMove(mInboundDir, mFilePattern,
                        mProcessDir, OBJECT_MAPPER));

            } else if ("WMQ".equalsIgnoreCase(mAdapterFlag)) {
                yLOGGER.log(Level.INFO, LOG_PREFIX + "getNewTransactionIds: WMQ mode: consuming messages from MQ");
                pIds.addAll(CsdBloombergPdAdapter.extractIdsFromMq(OBJECT_MAPPER));
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "getNewTransactionIds: Error retrieving new transaction IDs",
                    e);
    ...
    */
    private List<String> getPendingResponseIds() {
        ...
        try {
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "getPendingResponseIds: Error retrieving pending response IDs",
                    e);
    ...
    */
    ...
    public void updateRecord(String id, ServiceData serviceData, String controlItem,
        ...
        yLOGGER.log(Level.INFO, LOG_PREFIX + "updateRecord: Processing updateRecord for controlItem: {0}, ID: {1}",
        ...
        try {
            ...
            if (controlItem == null || controlItem.isEmpty()) {
                yLOGGER.log(Level.WARNING,
                        LOG_PREFIX + "updateRecord: Control item is null or empty, defaulting to PROCESS.PD");
    ...
    */
    private void processOfsRequest(String pRecordId, List<SynchronousTransactionData> pTransactionData,
        ...
        try {
            ...
            yLOGGER.log(Level.INFO, LOG_PREFIX + "processOfsRequest: Processing OFS request for record ID: {0}",
                    pRecordId);
            ...
            Map<String, String> pData = CsdBloombergPdMapper.mapPlacementsToPd(pItem);
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "processOfsRequest: Successfully prepared OFS transaction for PD update");
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "processOfsRequest: Transaction metadata cached for responseId={0}", pResponseId);
            ...
            yLOGGER.log(Level.SEVERE, ioe, () -> LOG_PREFIX + "processOfsRequest: I/O error for id=" + pRecordId);
    ...
    */
    private void checkOfsResponse(String pResponseId) {
        ...
        try {
            ...
            if (pOfsRequestDetailRecord != null && pOfsRequestDetailRecord.getMsgOut() != null) {
                ...
                if (pMsgOut != null && !pMsgOut.isEmpty()) {
                    ...
                    if (pMsgOut.contains("/-1/")) {
                        ...
                        yLOGGER.log(Level.INFO, LOG_PREFIX + "checkOfsResponse: Transaction successful: ref={0}",
                                pTransactRef);
                    }
                    ...
                    if (MSG_SUCCESS.equalsIgnoreCase(pStatus) && "WMQ".equalsIgnoreCase(pMetadata.adapterMode)) {
                        CsdBloombergPdAdapter.acknowledgeMqMessage(pMetadata.originalId);
                        yLOGGER.log(Level.INFO,
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "checkOfsResponse: Unexpected error in checkOfsResponse", e);
    ...
    */
    private JsonNode processFileRecord(String pId) throws java.io.IOException {
        FileItemRef pRef = CsdBloombergPdAdapter.parseFileItemRef(pId);
        if (pRef == null) {
        ...
        JsonNode pRoot = CsdBloombergPdAdapter.readRoot(pRef.file(), OBJECT_MAPPER);
        if (!CsdBloombergPdMapper.hasPlacements(pRoot)) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processFileRecord: FILE mode: no PLACEMENTS in {0}",
                    pRef.file().getFileName());
            return null;
        }

        return CsdBloombergPdMapper.getPlacementsAt(pRoot, pRef.index());
    }
    ...
    */
    private JsonNode processWmqRecord(String pId) throws java.io.IOException {
        MqItemRef pRef = CsdBloombergPdAdapter.parseMqItemRef(pId);
        if (pRef == null) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: unexpected id format: {0}", pId);
            return null;
        }

        JsonNode pRoot = CsdBloombergPdAdapter.readMqMessage(pRef.messageId(), OBJECT_MAPPER);
        if (pRoot == null || !CsdBloombergPdMapper.hasPlacements(pRoot)) {
            yLOGGER.log(Level.WARNING, LOG_PREFIX + "processWmqRecord: WMQ mode: no PLACEMENTS in message {0}",
                    pRef.messageId());
            return null;
        }
    ...
    private String buildResponseId(String pId) {
        if ("WMQ".equalsIgnoreCase(mAdapterFlag)) {
            ...
            if (pRef != null && pRef.messageId() != null) {
                ...
                yLOGGER.log(Level.INFO, LOG_PREFIX + "buildResponseId: Using JMS Message ID as responseId: {0}",
                        pJmsMessageId);
    ...
    */
    private boolean buildPdRecord(String pResponseId, Map<String, String> pData,
        ...
        try {
            ...
            yLOGGER.log(Level.INFO,
                    LOG_PREFIX + "buildDepoRecord: Building Deposit Placements record for responseId: {0}",
                    pResponseId);
            ...
            if (sCustomerNo.isEmpty() || sCurrency.isEmpty() || sPrincipal.isEmpty()) {
                yLOGGER.log(Level.SEVERE, "[CsdBloombergPdService] Missing required fields");
                return false;
            }
            // Log extracted field values
            yLOGGER.log(Level.INFO,
                    "[CsdBloombergPdService] Extracted fields - CNUM={0}, TCCY={1}, PPPL={2}, ERTE={3}, ALCY={4}",
            ...
            yLOGGER.log(Level.INFO, "[CsdBloombergPdService] Adding record: {0}", pPlaceDepoRecord);
            ...
            yLOGGER.log(Level.SEVERE, LOG_PREFIX + "buildPdRecord: Error building CbnPlacementDepositRecord", e);
            return false;
        } finally {
            // Cleanup
        ...
        try {
            if (pOfsResponse.contains("/") && pOfsResponse.contains(",")) {
                ...
                if (firstSlash > 0 && secondSlash > firstSlash && commaAfter > secondSlash) {
                    ...
                    yLOGGER.log(Level.INFO, LOG_PREFIX + "parseTransactionRef: Parsed transaction reference: {0}",
                            pTxnRef);
                    return pTxnRef;
                }
            }
            ...
            yLOGGER.log(Level.WARNING,
                    LOG_PREFIX + "parseTransactionRef: Error parsing transaction reference from OFS response", e);
            return MSG_UNKNOWN;
        }
    }

    /**
    * Builds response and publishes using CsdBloombergPdPayload and CsdBloombergPdProducer.
    */
    ...
    private void persistToExcepts(String pId, String pReason) {
        try {
            if ("FILE".equalsIgnoreCase(mAdapterFlag)) {
                FileItemRef pRef = CsdBloombergPdAdapter.parseFileItemRef(pId);
                if (pRef != null) {
                    CsdBloombergPdAdapter.persistFailedFileItem(pRef, mExceptsDir, pReason);
                } else {
                    yLOGGER.log(Level.WARNING,
                ...
                MqItemRef pRef = CsdBloombergPdAdapter.parseMqItemRef(pId);
                if (pRef != null) {
                    CsdBloombergPdAdapter.persistFailedMqItem(pRef, mExceptsDir, pReason);
                } else {
                    yLOGGER.log(Level.WARNING,
