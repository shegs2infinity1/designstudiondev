...
/**
 * Title: CbnFtConsumer.java Author: CSD Development Team Date Created:
 * 2025-10-11
 * 
...
public final class CbnFtConsumer {

    // ==== STATIC CONSTANTS ====
    private static final Logger LOGGER = Logger.getLogger(CbnFtConsumer.class.getName());

    /**
     * Private constructor to prevent instantiation of utility class.
     */
    private CbnFtConsumer() {
    ...
    public static MQQueue createQueue(String pQueueName) throws JMSException {
        ...
        LOGGER.log(Level.FINE, "[CbnFtConsumer] Created MQ queue: {0}", pQueueName);
        return queue;
    }
    ...
    public static void sendTextMessage(Connection pConnection, String pQueueName, String pMessageText)
        ...
        try {
            ...
            try {
                ...
                LOGGER.log(Level.WARNING, "[CbnFtConsumer] Failed to set encoding properties", e);
            }

            producer.send(message);
            LOGGER.log(Level.INFO, "[CbnFtConsumer] Sent message to queue: {0}", pQueueName);
    ...
    public static int consumeMessages(Connection pConnection, String pQueueName, MessageProcessor pProcessor,
        ...
        LOGGER.log(Level.INFO, "[CbnFtConsumer] Starting consumption from queue: {0}", pQueueName);
        ...
        try {
            ...
            while (continueConsuming) {
                ...
                if (pMaxMessages > 0 && consumedCount >= pMaxMessages) {
                    LOGGER.log(Level.INFO, "[CbnFtConsumer] Max messages reached: {0}", pMaxMessages);
                    break;
                }
                ...
                if (message == null) {
                    LOGGER.log(Level.INFO, "[CbnFtConsumer] No more messages available (timeout)");
                    break;
                }
                ...
                try {
                    ...
                    LOGGER.log(Level.FINE, "[CbnFtConsumer] Processing message: {0}", messageId);
                    ...
                    consumedCount++;
                    LOGGER.log(Level.FINE, "[CbnFtConsumer] Successfully processed message {0}/{1}",
                            new Object[] { consumedCount, pMaxMessages > 0 ? pMaxMessages : "unlimited" });

                } catch (Exception e) {
                    LOGGER.log(Level.SEVERE, "[CbnFtConsumer] Error processing message", e);
                    // Continue consuming despite processing error
                }
            }

            LOGGER.log(Level.INFO, "[CbnFtConsumer] Consumption complete. Total messages: {0}", consumedCount);
    ...
    private static String extractMessageBody(Message pMessage) throws JMSException {
        if (pMessage instanceof TextMessage) {
            ...
            LOGGER.log(Level.WARNING, "[CbnFtConsumer] Non-text message received: {0}",
    ...
    private static void closeQuietly(MessageProducer pProducer) {
        if (pProducer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnFtConsumer] Error closing producer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(MessageConsumer pConsumer) {
        if (pConsumer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnFtConsumer] Error closing consumer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(Session pSession) {
        if (pSession != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CbnFtConsumer] Error closing session (ignored)", e);
            }
        }
    }
}