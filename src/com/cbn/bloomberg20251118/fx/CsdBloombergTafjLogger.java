 ...
 * TafjLoggerHelper
 * 
 * This version preserves the existing flow and structure. Only enhancement:
 * picks base log folder and suffix from tmb-logger.properties.
 * 
 ...
 */
public final class CsdBloombergTafjLogger {

    private CsdBloombergTafjLogger() {
        // Utility class
    }
    ...
    public static Logger forClass(Class<?> owner) {
        ...
        synchronized (log) {
            ...
            try (InputStream in = CsdBloombergTafjLogger.class.getResourceAsStream("/tafjlogging.properties")) {
                if (in != null) {
                    logProps.load(in);
                }
            } catch (IOException e) {
            ...
            final String logsPath = ((tafjHome != null && !tafjHome.trim().isEmpty()) ? tafjHome : defaultFolder)
                    + pathSuffix;
            final String pattern = java.nio.file.Paths
                    .get(logsPath, owner.getSimpleName() + "_" + LocalDate.now() + "_%g.log").toString();
            // === END enhancement ===
