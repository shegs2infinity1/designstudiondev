...
/**
 * Title: CsdBloombergPdConsumer.java Author: CSD Development Team Date Created: 2025-10-11
 * 
...
public final class CsdBloombergPdConsumer {

    // ==== STATIC CONSTANTS ====
    private static final Logger LOGGER = Logger.getLogger(CsdBloombergPdConsumer.class.getName());

    /**
    * Private constructor to prevent instantiation of utility class.
    */
    private CsdBloombergPdConsumer() {
    ...
    */
    public static MQQueue createQueue(String pQueueName) throws JMSException {
        ...
        LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Created MQ queue: {0}", pQueueName);
        return queue;
    }
    ...
    public static void sendTextMessage(Connection pConnection, String pQueueName, String pMessageText)
        ...
        try {
            ...
            try {
                ...
                LOGGER.log(Level.WARNING, "[CsdBloombergPdConsumer] Failed to set encoding properties", e);
            }

            producer.send(message);
            LOGGER.log(Level.INFO, "[CsdBloombergPdConsumer] Sent message to queue: {0}", pQueueName);
    ...
    */
    public static int consumeMessages(Connection pConnection, String pQueueName, MessageProcessor pProcessor,
        ...
        LOGGER.log(Level.INFO, "[CsdBloombergPdConsumer] Starting consumption from queue: {0}", pQueueName);
        ...
        try {
            ...
            while (continueConsuming) {
                ...
                if (pMaxMessages > 0 && consumedCount >= pMaxMessages) {
                    LOGGER.log(Level.INFO, "[CsdBloombergPdConsumer] Max messages reached: {0}", pMaxMessages);
                    break;
                }
                ...
                if (message == null) {
                    LOGGER.log(Level.INFO, "[CsdBloombergPdConsumer] No more messages available (timeout)");
                    break;
                }
                ...
                try {
                    ...
                    LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Processing message: {0}", messageId);
                    ...
                    consumedCount++;
                    LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Successfully processed message {0}/{1}",
                            new Object[] { consumedCount, pMaxMessages > 0 ? pMaxMessages : "unlimited" });

                } catch (Exception e) {
                    LOGGER.log(Level.SEVERE, "[CsdBloombergPdConsumer] Error processing message", e);
                    // Continue consuming despite processing error
                }
            }

            LOGGER.log(Level.INFO, "[CsdBloombergPdConsumer] Consumption complete. Total messages: {0}",
                    consumedCount);
    ...
    */
    private static String extractMessageBody(Message pMessage) throws JMSException {
        if (pMessage instanceof TextMessage) {
            ...
            LOGGER.log(Level.WARNING, "[CsdBloombergPdConsumer] Non-text message received: {0}",
    ...
    private static void closeQuietly(MessageProducer pProducer) {
        if (pProducer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Error closing producer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(MessageConsumer pConsumer) {
        if (pConsumer != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Error closing consumer (ignored)", e);
            }
        }
    }
    ...
    private static void closeQuietly(Session pSession) {
        if (pSession != null) {
            try {
                ...
                LOGGER.log(Level.FINE, "[CsdBloombergPdConsumer] Error closing session (ignored)", e);
            }
        }
    }
}