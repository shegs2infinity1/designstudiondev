 ...
 */
public final class CbnFxConsumer {

    // ==== STATIC CONSTANTS ====
    private static final Logger yLogger = Logger.getLogger(CbnFxConsumer.class.getName());

    /**
     * Private constructor to prevent instantiation of utility class.
     */
    private CbnFxConsumer() {
    ...
    public static MQQueue createQueue(String pQueueName) throws JMSException {
        ...
        yLogger.log(Level.FINE, "[CbnFxConsumer] Created MQ queue: {0}", pQueueName);
        return queue;
    }
        ...
        try {
            ...
            try {
                ...
                yLogger.log(Level.WARNING,
                        "[CbnFxConsumer] Failed to set encoding properties", e);
            }

            producer.send(message);
            yLogger.log(Level.INFO, "[CbnFxConsumer] Sent message to queue: {0}",
                    pQueueName);
    ...
    public static int consumeMessages(Connection pConnection, String pQueueName,
        ...
        yLogger.log(Level.INFO, "[CbnFxConsumer] Starting consumption from queue: {0}",
                pQueueName);
        ...
        try {
            ...
            while (continueConsuming) {
                ...
                if (pMaxMessages > 0 && consumedCount >= pMaxMessages) {
                    yLogger.log(Level.INFO, "[CbnFxConsumer] Max messages reached: {0}",
                            pMaxMessages);
                    break;
                }
                ...
                if (message == null) {
                    yLogger.log(Level.INFO,
                            "[CbnFxConsumer] No more messages available (timeout)");
                    break;
                }
                ...
                try {
                    ...
                    yLogger.log(Level.FINE, "[CbnFxConsumer] Processing message: {0}",
                            messageId);
                    ...
                    yLogger.log(Level.FINE,
                            "[CbnFxConsumer] Successfully processed message {0}/{1}",
                            new Object[] { consumedCount,
                                    pMaxMessages > 0 ? pMaxMessages : "unlimited" });

                } catch (Exception e) {
                    yLogger.log(Level.SEVERE, "[CbnFxConsumer] Error processing message",
                            e);
            ...
            yLogger.log(Level.INFO,
                    "[CbnFxConsumer] Consumption complete. Total messages: {0}",
                    consumedCount);
    ...
    private static String extractMessageBody(Message pMessage) throws JMSException {
        if (pMessage instanceof TextMessage) {
            ...
            yLogger.log(Level.WARNING, "[CbnFxConsumer] Non-text message received: {0}",
    ...
    private static void closeQuietly(MessageProducer pProducer) {
        if (pProducer != null) {
            try {
                ...
                yLogger.log(Level.FINE, "[CbnFxConsumer] Error closing producer (ignored)",
                        e);
            }
        }
    }
    ...
    private static void closeQuietly(MessageConsumer pConsumer) {
        if (pConsumer != null) {
            try {
                ...
                yLogger.log(Level.FINE, "[CbnFxConsumer] Error closing consumer (ignored)",
                        e);
            }
        }
    }
    ...
    private static void closeQuietly(Session pSession) {
        if (pSession != null) {
            try {
                ...
                yLogger.log(Level.FINE, "[CbnFxConsumer] Error closing session (ignored)",
                        e);
            }
        }
    }
