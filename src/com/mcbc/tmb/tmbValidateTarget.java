 ...
 *
 *
 * * /** * @category EB.API - V.VAL.DEFAULT.TARGET.TMB
 * 
 * @param Application
 *            - CUSTOMER
 ...
 *
 */
public class tmbValidateTarget extends RecordLifecycle {
    ...
    public TValidationResponse validateField(String application, String recordId, String fieldData, TStructure record) {
        ...
        if (debugg)
            System.out.println("Entered Validate Field routine ");

        try {
            TargetId = fieldData;
            TargetId = TargetId.trim();
            lrTVA = custRec.getLocalRefField("L.TVA").toString();
            lrRSC = custRec.getLocalRefField("L.RSC").toString();
            tflrTVA = custRec.getLocalRefField("L.TVA");
            tflrRSC = custRec.getLocalRefField("L.RSC");
            String tvaOvr = tflrTVA.getOverride().toString();
            String tsaOvr = tflrRSC.getOverride().toString();
            String RsvFlag = custRec.getReserved51().toString();
            if (debugg)
                System.out.println("Reserved Field " + RsvFlag );
            if (tvaOvr.length() > 0 || tsaOvr.length() > 0 || RsvFlag.length() > 0)
                ovrset = true;
            if (debugg)
            ...
            if (!ovrset) {
                ...
                if (debugg) {
                    ...
                    System.out.println("Value of lrsc " + lrsc);
                    custRec.setReserved51("T");
                }
            }
